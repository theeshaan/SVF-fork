# Flow insesnitive points to info extractor for SVF
...
def parse_pta_input(file_path):
    ...
    matches = re.findall(r"Ptr (\d+)\s+PointsTo: {(.*)}", content) # {empty} sets don't get captured as no space after opening brace

    for match in matches:
        pointer_id, pointees = match
        graph[int(pointer_id)] = {int(pointee) for pointee in pointees.strip().split() if pointees.strip().split()[0] != "empty"}
...
def remove_dead_nodes(graph, token_map, func_name_map):
    ...
    # Pass 3: Remove functions from graph
    for node_id in list(graph.keys()):
        if token_map[node_id][0] in func_name_map.values():
            graph.pop(node_id)
...
def display_pts_to_info(graph, id_map):
    print(f"{'Pointer':<60} {'Pointees'}")
    for pointer, pointees in graph.items():
        if pointer not in id_map:
            continue
        
        pointee_names = [id_map[p] for p in pointees if p in id_map]
        print(f"{id_map[pointer]:<60} {{{', '.join(pointee_names)}}}")
...
if __name__ == "__main__":
    ...
    remove_dead_nodes(pta_graph, token_map, func_name_map)
